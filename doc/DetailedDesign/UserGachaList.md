...
- **在庫状況**: gacha_results（在庫数SSEでリアルタイム更新）、gacha_items.stock（初期在庫数）
...
- **GET /api/gachas** ... 公開ガチャ一覧取得（検索・絞り込み対応）**【✅ 実装完了】**
  - クエリパラメータ：
    ...
    - filter: フィルター（all, inStock, endingSoon）**【✅ 実装完了】**
...
- **エンドポイント例**: `/api/gachas/stock/stream`
